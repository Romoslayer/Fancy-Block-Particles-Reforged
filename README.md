<p align="center"><img src="Common/src/main/resources/logo.png" alt="Fancy Block Particles Reforged" width="256" /></p>

# Fancy Block Particles Reforged
This is **Romoslayer's** port of *Fancy Block Particles* to **Minecraft 26.3** (Fabric) and **26.2** (Fabric & NeoForge), built on top of Hantonik and TominoCZ's *FBP Renewed*, which itself is a revamp of MorphoxL's original *Fancy Block Particles* mod. See [Credits](#credits) below for the full lineage.

## Downloads

[CurseForge](https://www.curseforge.com/minecraft/mc-mods/fancy-block-particles-reforged) · [Modrinth](https://modrinth.com/mod/fbp-reforged)

Each Minecraft version lives on its own branch: [`26.3`](https://github.com/Romoslayer/Fancy-Block-Particles-Reforged/tree/26.3) (Fabric only, until NeoForge ships 26.3) and [`26.2`](https://github.com/Romoslayer/Fancy-Block-Particles-Reforged/tree/26.2) (Fabric & NeoForge).

Looking for the officially published, actively-maintained mod on older versions instead? Get it from Hantonik's pages:\
[CurseForge](https://curseforge.com/minecraft/mc-mods/fbp-renewed "Then just click download ;)") · [Modrinth](https://modrinth.com/mod/fbp-renewed "Then just click download ;)")

## Contribution

**Found a bug in this port?** Report it [here](https://github.com/Romoslayer/Fancy-Block-Particles-Reforged/issues/new "Enter all the necessary information!"), and I'll try to fix it!\
This is just a port — no new features are planned, so feature requests should go to the upstream project instead.

## Code

This project is **open source** — feel free to use the code, but please credit the original authors (see [Credits](#credits) below).

## Credits

This mod stands on the shoulders of the people who built and maintained it long before this port existed:

- **[MorphoxL](https://www.curseforge.com/minecraft/mc-mods/fancy-block-particles)** — created the original *Fancy Block Particles* mod for Minecraft 1.12.2, the foundation this whole project is built on.
- **[Hantonik](https://github.com/Hantonik) & [TominoCZ](https://github.com/TominoCZ)** — authors of *[FBP Renewed](https://github.com/Hantonik/FancyBlockParticles)*, the actively-maintained continuation of the mod that this port was forked and ported from. All license terms (GPLv3) and their original authorship are preserved here — go support their work using the links below if you can!
- **Romoslayer** — ported this fork to Minecraft 26.2 and 26.3.

## Support

If you enjoy this mod, consider supporting the people doing the ongoing work on the upstream project:

<a title="Thanks!" href="https://www.buymeacoffee.com/hantonik"><img src="https://img.buymeacoffee.com/button-api/?text=Buy me a coffee&amp;emoji=&amp;slug=hantonik&amp;button_colour=BD5FFF&amp;font_colour=ffffff&amp;font_family=Cookie&amp;outline_colour=000000&amp;coffee_colour=FFDD00" /></a>
