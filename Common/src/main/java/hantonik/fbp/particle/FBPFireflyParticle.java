...
public class FBPFireflyParticle extends FireflyParticle implements IKillableParticle {
            ...
            if (!FancyBlockParticles.CONFIG.global.isFreezeEffect()) {
                ...
                if (this.onGround) {
                    ...
                    if (this.random.nextFloat() > 0.99F || !this.moved) {
                        ...
                        var speed = FBPRenderHelper.rotate(new Vector3f((float) this.xd, (float) this.yd, (float) this.zd), 0.0F, this.angleY, 0.0F);
    ...
    private void putCube(QuadParticleRenderState renderState, float x, float y, float z, float scale, float angleY, float u, float v, int light, float rCol, float gCol, float bCol, float alpha) {
        ...
        for (var i = 0; i < FBPConstants.CUBE_NORMALS.length; i++) {
            var normal = FBPConstants.CUBE_NORMALS[i].rotateY(angleY, new Vector3f());
            ...
            if (i < 2)
                new Quaternionf().rotateY(angleY).mul(faceRotation, faceRotation);
